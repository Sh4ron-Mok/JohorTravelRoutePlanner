...
import com.ai180183.johortravelrouteplanner.SuffixTree.*;
...
public class SuffixTreeHelper {
    ...
    private static int[] constructSuffixArray(String txt) {
        ...
        for (int offset=1; offset<n; offset+=offset) {
            ...
            for (int i=0; i<n; i++) {
                tmp[idx[i]] = ((i>0 && rnk[idx[i]] == rnk[idx[i-1]] && rnk[idx[i]+offset]
                        == rnk[idx[i-1]+offset]) ? tmp[idx[i-1]] : i);
            }
    ...
    public static Node constructSuffixTree(String txt) {
        ...
        for (int i=0; i<suffixArray.length; i++) {
            ...
            if (curNode.depth == lcp[i]) {
                ...
                SuffixTree.Edge e2 = new SuffixTree.Edge();
                ...
                SuffixTree.Node leaf = new SuffixTree.Node();
                leaf.parent = internal;
                leaf.depth = n-suffixArray[i];
                leaf.suffix = suffixArray[i];

                SuffixTree.Edge e3 = new SuffixTree.Edge();
